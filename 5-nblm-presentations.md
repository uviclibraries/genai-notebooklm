...
## Activity Outline
- Use the notebook from the first activity with the 3 badging journal articles for this activity (explain how to get back to it?)
- In the chat box at the bottom of the page type: How useful are informal credentials in helping students find jobs?
  - Review the summary to make sure there are not mistakes or inaccuracies.
- In the chat box at the bottom of the page type: Create a google slides presentaion summary.
  - Review the presentation text to make sure there are not mistakes or inaccuracies.
- In the chat box at the bottom of the page type: Create an image for each slide.
  - Other prompt: Create an image for each slide in an abstract style, with a consistent colour pallet across all the image.
- Go to Meta.ai to create the images (as of December 2024 the free Google Gemini image creator is not nearly as good as the free Meta.ai image generator:
